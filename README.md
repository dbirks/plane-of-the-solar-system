...
- The pull-out looks down at **your own yellow dot** from the first kilometres up, and the gaze then never re-aims: the Moon and the whole system enter the frame purely by the field of view widening, so the journey never spins
...
- The **plane of the solar system is drawn at every scale**: a subtle ecliptic band across the real sky from the ground (the arc the Sun, Moon, and planets ride), Earth's own orbit line around the Sun, and the faint ecliptic rings at system scale
...
- **Point with phone**: opt-in device orientation drives both heading and pitch, so aiming your phone at the sky looks there
...
- 83 unit tests, 44 Playwright scenarios (desktop + mobile), and GitHub Pages deployment
