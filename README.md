...
- Screen-space **arrow markers** with click-to-look for every bright body — a small neutral arrow whose tip rests on the body when it's too far to resolve (an arrow suggests no size), label alone once its disc is plainly visible, a hollow circle when ghosted below the horizon, and edge-pinned arrows that rotate to point off-screen toward the body, named
...
- A two-sentence **first-visit welcome**, and a single **Settings dialog** behind the header's sliders icon: how to move, pointing with your phone, the optional guide geometry, and data credits; the header title tracks your landmark live, and altitude readouts include your real ground elevation
- The pull-out is **one continuous motion**: from about 10 km up the frame eases straight into the reveal — the plane of the solar system lying flat across the background and **Earth standing off to the right of the frame with your own blue dot on its side** — with no nadir detour and nothing new starting at low orbit; from there to 80 AU the journey only zooms
...
- A Guides section (in Settings) for optional explanation geometry: orbits, ecliptic rings, Moon orbit, sunlight direction, Earth axis & equator, sky grid, labels — sparse by default
- Marker labels declutter automatically when bodies crowd; adaptive pixel-ratio under sustained slow frames
- **Compass mode**: opt-in device orientation drives the camera as a full attitude quaternion, so aiming your phone at the sky looks there — including straight up through the zenith without the flip that heading-plus-pitch controls suffer
